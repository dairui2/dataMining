...
频繁规则：关联结果中支持度Support = P((A,B)|ALL) = P(B&A)/P(ALL)和置信度Confidence = P(B|A) = P(B&A)/P(A)都比较高的规则。
有效规则：关联规则中真正能促进规则中的前(A)/后(B)项的提升。
提升度：Lift = P(B|A)/P(B)。当提升度为1时，说明应用关联规则和不应用关联规则产生相同的结果；当提升度大于1时，说明应用关联规则比不应用关联规则产生更好的结果；
